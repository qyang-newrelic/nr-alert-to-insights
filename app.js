...
function getCurrentNotifications() {
	...
	var res = request('https://api.newrelic.com/v2/alerts_channels.json', options);
...
function createNotificationChannel(notificationList){
	...
	var createNewChannelUrl = 'https://api.newrelic.com/v2/alerts_channels.json';
	var opts = {
			method: 'POST',
			   headers: {
			       'X-Api-Key': restAdminKey,
...
function getPolicyList(){
	...
	var res = request('https://api.newrelic.com/v2/alerts_policies.json', options);
	if (res.err || res.status != 200) {
...
function bindNotificationToPolicies(channelId, policyList){
	...
	var linkChannelUrl = 'https://api.newrelic.com/v2/alerts_policy_channels.json';
...
function hasDashboardDeployed(){
	...
	var options = {
		...
		var res = request('https://api.newrelic.com/v2/dashboards.json', options);
		if (res.err || res.status != 200) {
			console.log("error requesting policies: ", res.err, res.status);
			throw new Error("Error retrieving legacy alert policies");
		}

		console.log(res.data.dashboards.length);
...
function deployDashboard(){
	...
	console.log('try dashfile');
	var dashfile = JSON.parse(fs.readFileSync( __dirname + '/new_dashboard.json'));
